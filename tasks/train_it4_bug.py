...
from models import *
from tasks.tmp.shared_utils_new import get_media_types, setup_model
...
def train(
    ...
    save_iter=config.get("save_iter", 0)
    ...
    for i, (media_type, (image, text, instruction, _)) in enumerate(iterator):
        ...
        if is_main_process() and global_step % save_iter == 0:
            print("Save model in step ", global_step, "!")
            param_grad_dic = {
...
def main(config):
    ...
    for epoch in range(start_epoch, config.scheduler.epochs):
        skip_iter = global_step - epoch * num_steps_per_epoch
        logger.info(f"Epoch {epoch} : Skip iter {skip_iter} start ! ")
