...
class TemporalAdapter(nn.Module):
    def __init__(self, merge_len, clip_num, input_dim, mid_dim, output_dim):
        super().__init__()
        self.AvgPool = nn.AvgPool1d(merge_len, stride = merge_len)
        self.upsample = nn.Upsample(scale_factor = 4)
        
        self.linear_input = nn.Linear(input_dim, mid_dim)
        self.linear_output = nn.Linear(mid_dim, output_dim)
        
        nn.init.constant_(self.linear_output.weight, 0)
        nn.init.constant_(self.linear_output.bias, 0)
        
        self.DwConv1 = nn.Sequential(
            nn.Conv1d(mid_dim, mid_dim, merge_len*2+1, stride=4, padding=merge_len, groups=mid_dim),
            nn.Conv1d(mid_dim, mid_dim, 1),
            TransposeLayerNorm(mid_dim),
            nn.GELU(),
        )
        ...
        self.fc = nn.Sequential(
            nn.Conv1d(mid_dim, mid_dim, clip_num+1, stride=1, padding=clip_num//2),
            TransposeLayerNorm(mid_dim),
            nn.GELU(),
        )
        self.UpConv1 = nn.Sequential(
            nn.Conv1d(mid_dim, mid_dim, merge_len+1, stride=1, padding=merge_len//2, groups=mid_dim),
            nn.Conv1d(mid_dim, mid_dim, 1),
            TransposeLayerNorm(mid_dim),
            nn.GELU(),
        )
    ...
    def forward(self, input_tokens):
        time_ad = self.linear_input(input_tokens).transpose(1, 2)
        time_ad0 = self.AvgPool(time_ad)
        time_ad1 = self.DwConv1(time_ad0)
        time_ad2 = self.DwConv2(time_ad1)
        time_ad2 = self.fc(time_ad2)
        time_ad1 = self.upsample(time_ad2)+ time_ad1
        time_ad1 = self.UpConv1(time_ad1)
        time_ad0 = self.upsample(time_ad1)+ time_ad0
        time_ad = self.UpConv0(time_ad0)
        time_ad = self.linear_output(time_ad.transpose(1, 2))
        return time_ad
...
class VideoChat2_it4_mistral_LinearProAda(Blip2Base):
    """
    VideoChat2 model.
    """
    def __init__(self, config):
        ...
        # #Quantize
        # self.LLM_bits="8bit"
        # if self.LLM_bits in ['4bit','8bit']:
        #     print(" !!! Trans LLM to BNB with bits:",self.LLM_bits)
        #     from utils.quantization import quant_model_bnb
        #     self.mistral_model.model.layers=quant_model_bnb(self.mistral_model.model.layers,quant_bit=self.LLM_bits)
        
        #Timechat tranfer
        self.clip_frames = config.clip_frames
        self.total_frames = config.num_frames
        assert (self.total_frames % self.clip_frames == 0)
        self.clip_num = self.total_frames // self.clip_frames
        self.token_merge_len = config.token_merge_len
        ...
        self.quant=False
        if self.quant:
            logger.info("quantize Mistral to 4bit!")
            self.quantize(self.mistral_model)
        
        self.clip_proj = nn.Linear(
            self.qformer.config.hidden_size, self.qformer.config.hidden_size
        )
        
        self.merge_proj = nn.Linear(
            self.qformer.config.hidden_size*self.token_merge_len, self.mistral_model.config.hidden_size
        )
        ...
        self.temp_adap = TemporalAdapter(self.token_merge_len, self.clip_num, self.qformer.config.hidden_size, self.qformer.config.hidden_size//4, self.mistral_model.config.hidden_size)
        
        self.max_txt_len = max_txt_len
        
        # load weights of VideoChat2
        if videochat2_model_path:
            logger.info(f"Load VideoChat2 stage3 from: {videochat2_model_path}")
        ...
        if pretrained_path:
            logger.info(f"Load peretrained from: {pretrained_path}")
            ckpt = torch.load(pretrained_path, map_location="cpu")
            if 'model' in ckpt.keys():
                msg = self.load_state_dict(ckpt['model'], strict=False)
            elif 'module' in ckpt.keys():
                msg = self.load_state_dict(ckpt['module'], strict=False)
            else:
                msg = self.load_state_dict(ckpt, strict=False)
            logger.info(msg)
    ...
    def quantize(self, model):
        from utils.quant import quantize
    ...
    def encode_img(self, image, instruction):
        ...
        with self.maybe_autocast():
            ...
            inputs_mistral = self.clip_proj(query_output.last_hidden_state[:, :query_tokens.size(1), :])
        return inputs_mistral

    def _get_text_len(self, text):
        return self.mistral_tokenizer(text, return_tensors="pt", add_special_tokens=False).input_ids.shape[1]

    def get_clip_time_instruct(self,text_input):
        time_instructions=()
        for text in text_input:
            time_text = text.split("</Video>")[-1].split("[/INST]")[0]
            sec=time_text.split("at ")[-1].split(" seconds.")[0].split(", ")
            for i in range(self.clip_num):
                if(len(sec) < self.clip_num*self.clip_frames):
                    time_instru = f"The {i+1}-th video clip contains {self.clip_frames} frames without timestamps. "
                else:
                    time_instru = f"The {i+1}-th video clip contains {self.clip_frames} frames sampled at {', '.join(sec[i*self.clip_frames:(i+1)*self.clip_frames])} seconds. "
                time_instructions+=(time_instru,) 
        return time_instructions
    
    def encode_long_video(self, image, text_input, instruction):            
        B, T, C, H, W = image.shape
        time_instructions = self.get_clip_time_instruct(text_input)
        with self.maybe_autocast():
            image = image.reshape(-1, self.clip_frames, C, H, W)
            img_embeds = self.encode_img(image, time_instructions)
            img_embeds = img_embeds.reshape(B, -1, img_embeds.shape[2])
            
            temporal_info = self.temp_adap(img_embeds)
            
            batch_size, img_len, token_dim = img_embeds.shape
            img_embeds = img_embeds.view(batch_size, img_len // self.token_merge_len, self.token_merge_len * token_dim)
            
            img_embeds = self.merge_proj(img_embeds) + temporal_info
    ...
    def forward(self, image, text_input, instruction):
        ...
        if T==1:
            use_image=True
            img_embeds = self.encode_img(image, instruction)
            img_embeds = img_embeds.repeat(1, 1, self.token_merge_len)
            img_embeds = self.merge_proj(img_embeds)
        else:
            use_image=False
            img_embeds = self.encode_long_video(image, text_input, instruction)  
