import torch
from utils.peft import LoraColumnParallelLinear, LoraRowParallelLinear
...
def quantize(model, quant_conf : BitsAndBytesConfig):
    ...
    if hasattr(model, "get_quant_blocklist"):
        quant_blocklist = [x for x in model.get_quant_blocklist()]
    else:
        quant_blocklist = []

    for name, module in tqdm(module_list, desc="Qunatization Process",mininterval=10):
